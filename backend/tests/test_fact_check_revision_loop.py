...
class TestRevisionLoopIncludesFactCheck:
    ...
    def test_fact_check_appears_twice_in_full_pipeline_path(self):
        ...
        assert ("content_generation", "intro_ab_testing") in self.edges, (
            "content_generation must route to intro_ab_testing first"
        )
        ...
        assert ("sme_review_check", "engagement_check") in self.edges, (
            "sme_review_check must feed engagement_check (Sprint 31)"
        )
        assert ("engagement_check", "readability_check") in self.edges, (
            "engagement_check must feed readability_check (Sprint 31)"
        )
        assert ("readability_check", "quality_analysis") in self.edges, (
            "readability_check must always feed into quality_analysis"
        )
...
class TestFactCheckGraphConnectivity:
    ...
    def test_all_paths_to_quality_analysis_pass_through_fact_check(self):
        ...
        # readability_check is last in chain: fact_check → ai_slop_check → truth_enforcement → human_voice_check → line_edit_check → structure_check → copy_edit_check → sme_review_check → engagement_check → readability_check → quality_analysis
        assert predecessors == {"readability_check"}, (
            f"Only readability_check should feed quality_analysis directly, got: {predecessors}"
        )
