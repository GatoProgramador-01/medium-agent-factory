...
async def test_good_cohort_mean(good_cases: list[dict]) -> None:
    ...
    """
    scores = [
        (await _analyze(c)).score for c in good_cases[:4]  # first 4 to keep CI cost low
    ]
...
async def test_bad_cohort_mean(bad_cases: list[dict]) -> None:
    ...
    scores = [(await _analyze(c)).score for c in bad_cases[:4]]
    mean = statistics.mean(scores)
    assert mean <= 0.57, (
...
async def test_revision_prompt_is_specific(bad_cases: list[dict]) -> None:
    ...
    assert (
        len(report.revision_prompt) >= 100
